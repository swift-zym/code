...
void update_bfs(int k, int type) {
  ...
  while (!q.empty()) {
    node hd = q.front();
    q.pop();
    if ((hd.val != p[hd.k_max] && type >= 0) ||
        type == -1) {  // if(type==-1)cout << "****" << hd.val << "****" <<
                       // hd.f_val << endl;
      ll w = n / hd.val;
      w = n / w;  // cout << hd.val << "[" << w<<" , " << val_id[val_id_num] <<
                  // "]" << endl;
      if (type == -1) {
    ...
    for (int i = hd.k_max + 1; hd.val * p[i] < n / n_3 && i <= p_sz_2; i++) {
      ...
      for (int e = 1;; e++) {
        if (hd.val * res < n / n_3) {
          ...
          nxt.val = hd.val * res;
          if (type == -1)
            nxt.f_val = hd.f_val * f(p[i], e);
          else
            nxt.f_val = hd.f_val * poww(res, type);
          q.push(nxt);
        } else
          break;
        res *= p[i];
  ...
  for (int tt = 0; tt <= times; tt++) {
    ...
    for (; p[now] <= n_3; now++) {
      for (int j = val_id_num; j >= 1; j--) {
        ...
        if (w < n / n_3)
          ...
          f_p[j][tt] = f_p[j][tt] - (f_p[get_id(w)][tt] - sum(p[now - 1])) *
                                        poww(p[now], tt);
      }
      update_bfs(now, tt);
    }
    ...
    for (; now <= p_sz_2; now++) {
      for (int j = val_id_num; j >= 1; j--) {
        ...
        if (w < p[now]) break;
        f_p[j][tt] -=
            (f_p[get_id(w)][tt] - f_p[p[now - 1]][tt]) * poww(p[now], tt);
      }
    }
  }

  for (int i = 1; i <= val_id_num; i++) {
    /// if f(p) = p^2+3p+1,then write:f_p[i][0] = f_p[i][2] + 3*f_p[i][1] +
    /// f_p[i][0];
    f_p[i][0] = -f_p[i][1] + f_p[i][0];
  }
}
ll F[2000000 + 100];
void get_f_3(ll n) {  /// V(F_{pi(n^(1/3))+1},n)
  ...
  for (int now = 1; now <= val_id_num; now++) {
    if (val_id[now] < q) {
      ...
      F[now] = 1 + (f_p[now][0] - f_p[q - 1][0]);
      for (int pp = p_sz_3 + 1;
           p[pp] <= (int)(sqrt(val_id[now])) && pp <= p_sz_2; pp++) {
        F[now] +=
            f(p[pp], 2) + (f(p[pp], 1)) * (f_p[get_id(val_id[now] / p[pp])][0] -
                                           f_p[get_id(p[pp])][0]);
      }
    }
  }
